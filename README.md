...
That’s it—Franklin figures out which OS he’s visiting, downloads the right shell, and gently asks about your palette along the way. Need automation? Use the flags below.
...
| fzf, ripgrep | Included on Linux/RHEL shells for quick fuzzy search and grepping. |
...
Everything lives under `~/.config/franklin` (or your `--install-root`). Before touching your existing setup, the installer backs up `.zshrc`, `.zshenv`, `.zprofile`, Antigen, and `~/.config/starship.toml` to `~/.local/share/franklin/backups/<timestamp>`.

## Daily Moves

| Command | Purpose |
| --- | --- |
| `update-all.sh` | Updates OS packages (brew/apt/dnf), Antigen, Starship, NVM, Node, npm globals, and version pins. Each bundle keeps only its OS logic, so the output stays tidy. |
...
MIT License. `bootstrap.sh` and per-OS bundles live at [github.com/jeremyfuksa/franklin](https://github.com/jeremyfuksa/franklin).
